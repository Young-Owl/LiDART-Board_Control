...
#---------------------------------------------------------------------------------------------------------------------------------------
    ...
    '''
    if cnt < 100:
        value = ueye.c_int(int(2000))
        ueye.is_Gamma(hCam, ueye.IS_GAMMA_CMD_SET, value, ueye.sizeof(value))
    if cnt >= 100 and cnt < 200:
        print("new")
        value = ueye.c_int(int(100))
        ueye.is_Gamma(hCam, ueye.IS_GAMMA_CMD_SET, value, ueye.sizeof(value))
    if cnt >= 200 and cnt < 300:
        value = ueye.c_int(int(0))
        ueye.is_Gamma(hCam, ueye.IS_GAMMA_CMD_SET, value, ueye.sizeof(value))
    '''
    
    # Changing the gamma value
    value = ueye.c_int(int(500))
    ueye.is_Gamma(hCam, ueye.IS_GAMMA_CMD_SET, value, ueye.sizeof(value))
    
    # Changing the exposure value
    exposure = 1000000000e-6    # Camera Width (Mininum 18us)
    new_exposure = ueye.c_double(exposure)
    ueye.is_Exposure(hCam,
                               ueye.IS_EXPOSURE_CMD_SET_EXPOSURE,
                               new_exposure, 8)
    
    # Changing the gainboost status
    ueye.is_SetGainBoost(hCam, ueye.IS_SET_GAINBOOST_OFF)
    
    #print(ueye.is_SetColorMode(hCam, ueye.IS_GET_BITS_PER_PIXEL))
        
    # Camera defaults to the IS_CM_BGR8_PACKED color mode, which separates the image into 3 channels
    # Each channel with 8 bits.
    # print(u.is_SetColorMode(self.id, u.IS_GET_COLOR_MODE))
    
    #ueye.is_SetColorMode(hCam, ueye.IS_CM_MONO12)
    #print(ueye.is_SetColorMode(hCam, ueye.IS_GET_BITS_PER_PIXEL))
